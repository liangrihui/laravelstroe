...
class AddressController extends Controller
{
     ...
     * @return void
     */
    public function __construct(User $repository, Configuration $configRepository)
    {
...
//        $addresses = $this->userRepository->addresses;  用注入的依赖不能调用属性
//        dd($addresses);
    ...
    public function store(AddressRequest $request)
    {
        ...
        $this->userRepository->createUserAddress($request->except(['_token']));
        if (session()->pull('previous') == 'http://127.0.0.1/laravelstroe/public/index.php/checkout'){
            return redirect()->back() ->with('notificationText', '成功添加一个地址!!');
        }else{
